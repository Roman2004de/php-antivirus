...
- EventHandlerScanner: проверка обработчиков событий Bitrix из `b_module_to_module`.
...
## Bitrix DB EventHandlerScanner

EventHandlerScanner анализирует записи таблицы `b_module_to_module`, где Bitrix хранит зарегистрированные обработчики событий. Для каждой подозрительной записи создается виртуальный путь `bitrix-db://b_module_to_module/{ID}`.

Сканер проверяет опасные имена методов, динамические callback-строки, пустой или неизвестный `TO_MODULE_ID`, критичные события вроде `OnPageStart`, `OnBeforeProlog`, `OnBeforeUserLogin`, `OnBeforeIBlockElementUpdate`, `OnSaleOrderSaved`, `OnBeforeMailSend`, а также цепочки request -> sink.

При включенном `resolve_event_handler_code` модуль ищет исходный код обработчика в `local/php_interface/init.php`, `bitrix/php_interface/init.php`, `include.php` и `lib/**/*.php` соответствующего модуля. Найденный файл прогоняется через текущий файловый detector; если в нем есть risk findings, в event-report добавляется `bitrix_event_handler_file_suspicious` с `resolved_file` и списком detector signatures.

Findings обработчиков событий получают `target=db_event`, category `bitrix_db`, trace с `ID`, `FROM_MODULE_ID`, `MESSAGE_ID`, `TO_MODULE_ID`, `TO_CLASS`, `TO_METHOD`, `SORT`, `risk_reason`, `resolved_file` и теги `entity:db_event`, `engine:bitrix_db`, `risk:persistence`.
...
- сканирование обработчиков событий `b_module_to_module`;
- поиск и анализ файлов обработчиков событий;
...
- `--scan-events=Y|N`: проверять обработчики событий `b_module_to_module`, если DB-сканирование включено;
- `--resolve-event-code=Y|N`: искать и анализировать файлы обработчиков событий;
...
Smoke-test EventHandlerScanner:

```bash
php bitrix/modules/delement.antivirus/tests/bitrix_event_handler_scanner_smoke.php
```
