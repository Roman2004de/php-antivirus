...
Стадия: MVP с работающим сканированием, отчетами, карантином, CLI runner, AST-анализом PHP, taint-анализом request-to-sink, усиленным `.htaccess`-анализом, `common_strings` prefilter для regex pipeline, `normalized_hash` для текстовых файлов, FindingSuppressor для точечных false positive, EntropyAnalyzer для encoded payload, UrlExtractor для внешних URL и Known Malware Hash Database для SHA-256 индикаторов.
...
Known malware hash проверяется отдельным hash DB analyzer-слоем через prefix-index и full SHA-256 базу.
...
- full hash match создает `known_malware_hash_match` с `severity=critical`, `score=100`, `recommendation=quarantine`;
...
- добавлены настройки `enable_hash_db`, `malware_hashes_path`, `malware_hash_prefixes_path`;
